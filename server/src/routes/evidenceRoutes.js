...
const evidenceFileFilter = (req, file, cb) => {
  ...
  if (isFindingUpload) {
    if (file.mimetype !== 'application/pdf') {
      return cb(new AppError('Documents attached to a finding must be a PDF file.', 400), false);
    }
    return cb(null, true);
  }

  if (!ALLOWED_MIME_TYPES.has(file.mimetype)) {
    return cb(new AppError('Unsupported file type. Allowed: PDF, Word, Excel, PNG, JPEG.', 400), false);
  }
  return cb(null, true);
};

const evidenceUpload = createUpload('evidence', { fileFilter: evidenceFileFilter });
...
router
  ...
  .post(restrictTo('Admin', 'Lead Auditor', 'Auditor', 'Auditee'), evidenceUpload.single('file'), evidenceController.createOne);
